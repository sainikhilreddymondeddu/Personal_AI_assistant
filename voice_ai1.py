...
def ask_ai(q):
    try:
        ...
        # 🔧 REMOVE leading "AI:" if model adds it
        if response.lower().startswith("ai:"):
            response = response[3:].strip()

        return response[:400] or "I could not find an answer."
...
async def handle_command(text):
    ...
    if t == "open last app":
        app = memory["recent_activity"]["last_opened_app"]
        if app:
            open_desktop_app(app)
            speak(f"Opening {app}")
        else:
            speak("No app opened before.")
        return

    if t == "open last site":
        site = memory["recent_activity"]["last_opened_site"]
        if site:
            await browser.open_app(site)
            speak(f"Opening {site}")
        else:
            speak("No site opened before.")
        return
